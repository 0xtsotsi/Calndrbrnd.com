...
vi.mock("@calndrbrnd/app-store/stripepayment/lib/utils", () => ({
...
vi.mock("@calndrbrnd/ee/api-keys/lib/apiKeys", () => ({
...
describe("RetellAIService", () => {
  ...
  describe("generatePhoneNumberCheckoutSession", () => {
    it("should generate checkout session successfully", async () => {
      const { getStripeCustomerIdFromUserId } = await import("@calndrbrnd/app-store/stripepayment/lib/customer");
      const { getPhoneNumberMonthlyPriceId } = await import("@calndrbrnd/app-store/stripepayment/lib/utils");
      const stripe = (await import("@calndrbrnd/features/ee/payments/server/stripe")).default;
