...
export const applyTheme = function () {
  try {
    // This utility is a replica of @calndrbrnd/lib/webstorage.ts but we can reuse it because applyTheme is stringified and injected, so we can't have deps here
    const safeLocalStorage = {
      getItem: function (key: string) {
        try {
          // eslint-disable-next-line @calndrbrnd/eslint/avoid-web-storage
