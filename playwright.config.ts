...
const IS_EMBED_TEST = process.argv.some((a) => a.startsWith("--project=@calndrbrnd/embed-core"));
const IS_EMBED_REACT_TEST = process.argv.some((a) => a.startsWith("--project=@calndrbrnd/embed-react"));

const webServer: PlaywrightTestConfig["webServer"] = [
  {
    command:
      "NEXT_PUBLIC_IS_E2E=1 NODE_OPTIONS='--dns-result-order=ipv4first' yarn workspace @calndrbrnd/web start -p 3000",
    port: 3000,
    timeout: 60_000,
...
if (IS_EMBED_TEST) {
  ...
  webServer.push({
    command: "yarn workspace @calndrbrnd/embed-core dev",
    port: 3100,
    timeout: 60_000,
    reuseExistingServer: !process.env.CI,
  });
...
if (IS_EMBED_REACT_TEST) {
  ...
  webServer.push({
    command: "yarn workspace @calndrbrnd/embed-react dev",
    port: 3101,
    timeout: 60_000,
    reuseExistingServer: !process.env.CI,
  });
...
const config: PlaywrightTestConfig = {
  ...
  projects: [
    ...
    {
      name: "@calndrbrnd/embed-core--firefox",
    ...
    {
      name: "@calndrbrnd/embed-core--webkit",
      testDir: "./packages/embeds/",
      testMatch: /.*\.e2e\.tsx?/,
      expect: {
        timeout: DEFAULT_EXPECT_TIMEOUT,
    ...
    {
      name: "@calndrbrnd/embed-core--isMobile",
      testDir: "./packages/embeds/embed-core/",
      testMatch: /.*\.e2e\.tsx?/,
      expect: {
        timeout: DEFAULT_EXPECT_TIMEOUT,
